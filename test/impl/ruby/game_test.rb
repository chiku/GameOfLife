  ...
  describe Game do
    describe "when created with a pattern" do
      it "adds the passed contents as its cells" do
        ...
        expect(game.has_cell_at?(1, 1)).must_equal true
        expect(game.has_cell_at?(-1, 2)).must_equal true
      end
    end
    ...
    describe "when empty" do
      ...
      it "is a game" do
        expect(empty_game.instance_of?(Game)).must_equal true
      end

      it "has a cell count of zero" do
        expect(empty_game.cell_count).must_equal 0
      end
    end
    ...
    describe "after adding one cell" do
      ...
      it "has cell count of one" do
        expect(game.cell_count).must_equal 1
      end

      it "knows where the cell is present" do
        expect(game.has_cell_at?(1, 0)).must_equal true
        expect(game.has_cell_at?(1, 1)).must_equal false
      end
    end
    ...
    describe "after adding two cells" do
      ...
      it "has cell count of two" do
        expect(game.cell_count).must_equal 2
      end
    end
    ...
    describe "blinker" do
      ...
      describe "after a tick" do
        ...
        it "has three cells" do
          expect(blinker.cell_count).must_equal 3
        end

        [[0, -1], [0, 0], [0, 1]].each do |position|
          x, y = position.first, position.last
          it "has a cell at (#{x}, #{y})" do
            expect(blinker.has_cell_at?(x, y)).must_equal true
          end
        end
      end
    end
    ...
    describe "current generation cells" do
      it "can be visited" do
        ...
        expect(xs).must_equal [-1, 0, 1, 5]
        expect(ys).must_equal [ 0,-1, 0, 5]
      end

      it "can be collected" do
        game = Game.new.add_cell_at(-1, 0).add_cell_at(0, -1).add_cell_at(1, 0)
        expect(game.current_generation).must_equal [Cell.new(-1, 0), Cell.new(0, -1), Cell.new(1, 0)]
      end

      it "is not cached" do
        game = Game.new.add_cell_at(-1, 0).add_cell_at(-2, 0).add_cell_at(-3, 0).tick!
        expect(game.current_generation).must_equal [Cell.new(-2, -1), Cell.new(-2, 0), Cell.new(-2, 1)]
      end
    end
    ...
    describe "previous generation cells" do
      ...
      it "can be collected" do
        game = Game.new.add_cell_at(-1, 0).add_cell_at(0, -1).add_cell_at(1, 0).add_cell_at(5, 5).tick!
        expect(game.previous_generation).must_equal [Cell.new(-1, 0), Cell.new(0, -1), Cell.new(1, 0), Cell.new(5, 5)]
      end

      it "is not cached" do
        game = Game.new.add_cell_at(-1, 0).add_cell_at(-2, 0).add_cell_at(-3, 0).tick!.tick!
        expect(game.previous_generation).must_equal [Cell.new(-2, -1), Cell.new(-2, 0), Cell.new(-2, 1)]
      end
    end
    ...
    describe "cells to add" do
      it "are the cells that exist in current generation but not in previous" do
        game = Game.new.add_cell_at(-1, 0).add_cell_at(0, 0).add_cell_at(1, 0)
        game.tick!
        expect(game.cells_to_add).must_equal [Cell.new(0, -1), Cell.new(0, 1)]
      end

      it "don't include unchanged cell" do
        game = Game.new.add_cell_at(0, 0).add_cell_at(1, 0).add_cell_at(0, 1).add_cell_at(1, 1)
        game.tick!
        expect(game.cells_to_add).must_equal []
      end
    end
    ...
    describe "on serialization to JSON" do
      ...
      it "produces a JSON string representation of the current generation" do
        ...
        expect(current.count).must_equal 3
        expect(current.include?({"x" => 0, "y" => -1, "generation" => 1})).must_equal true
        expect(current.include?({"x" => 0, "y" =>  0, "generation" => 2})).must_equal true
        expect(current.include?({"x" => 0, "y" =>  1, "generation" => 1})).must_equal true
      end
      ...
      it "produces a JSON string representation of the previous generation" do
        ...
        expect(previous.count).must_equal 3
        expect(previous.include?({"x" => -1, "y" => 0, "generation" => 1})).must_equal true
        expect(previous.include?({"x" =>  0, "y" => 0, "generation" => 1})).must_equal true
        expect(previous.include?({"x" =>  1, "y" => 0, "generation" => 1})).must_equal true
      end
    end
  end
