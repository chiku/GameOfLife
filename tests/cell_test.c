...
{
	World *world = World_Initialize();
	Cell *cell = Cell_Initialize(world, 5, 4);
...
START_TEST (test_Cell_Knows_does_not_consider_far_away_cell_as_neighbours)
{
	...
	Cell_Initialize(world,  0,  2);	Cell_Initialize(world,  2,  2);
	Cell_Initialize(world,  2,  0);	Cell_Initialize(world,  2, -2);
	Cell_Initialize(world,  0, -2);	Cell_Initialize(world, -2, -2);
	Cell_Initialize(world, -2,  0);	Cell_Initialize(world, -2,  2);
...
{
	World *world = World_Initialize();
	Cell *cell = Cell_Initialize(world, 3, 4);
